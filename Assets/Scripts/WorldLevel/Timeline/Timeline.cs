...
public class WireTimeline : Timeline<WireTimelineValue>
{
    private const int N_MEASURES    = 5;
    private const int MEASURE_SIZE  = 5;
    ...
    public WireTimeline(int LoopLevel) : base(N_MEASURES * MEASURE_SIZE)
    {
    }

    public WireTimeline(int LoopLevel, int CursorIdx) : base(N_MEASURES * MEASURE_SIZE, CursorIdx)
    {
        _LoopLevel = LoopLevel;
    }
    ...
    public void SetWire(Wire iWire)
    {
        foreach(WireTimelineValue V in _CursorValues)
        {
            V._Wire = iWire;
        }
    }
    ...
    // !! autoflag this TL as a previous TL ( used by UI )
    public WireTimeline GetNestedTimeline()
    {
        var Result = new WireTimeline( _LoopLevel+1, _Cursor);
